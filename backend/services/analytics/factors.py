...
def _rolling_beta_r2(a: np.ndarray, f: np.ndarray, dates, ticker: str, factor: str):
    ...
    for idx in range(_WINDOW, len(dates)):
        ...
        try:
            ...
            print(f"Error calculating {factor} beta for {ticker}: {e}")
            continue
...
class FactorAnalytics:
    ...
    def get_factor_exposure_data(
        ...
        try:
            ...
            print(f"Found {len(dates_global)} dates from {min(dates_global)} to {max(dates_global)}")
            ...
            print(f"Generated {len(factor_exposures)} factor exposures and {len(r2_data)} R^2 records")
